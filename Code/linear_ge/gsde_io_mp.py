...
With multiprocessing CAN"T DO WITH POOL - HAS TO SPAWN PROCESS EARILER AND DEFINE OUTERAGENT
"""
...
def inner_loop(outer_agent, model_index, inner_learning_steps, inner_batch_size, env, device, T, save_model=False):
    ...
    inner_agent = CalvanoGradientAgent(0.1, 0.99, device)
...
if __name__ == "__main__":
    ...
    T = 5
    inner_learning_steps = 40
    inner_batch_size = 20
    outer_learning_steps = 10
    scale = 0.05
    ...
    for outer_it in tqdm(range(outer_learning_steps)):
        ...
        model_indices = list(range(outer_agent.no_models))
        rewards = [0 for item in model_indices]

        def inner_mp(model_index):
            outer_reward, inner_reward = inner_loop(
        ...
        with concurrent.futures.ProcessPoolExecutor(max_workers=4) as executor:
            for model_index, reward in zip(model_indices, executor.map(inner_mp, model_indices)):
                rewards[model_index] = reward
