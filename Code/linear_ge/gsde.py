...
class CalvanoAgent(object):
    ...
    def clip_gradients(self, clip):
        l2_norm = sum(g.pow(2.0).mean()
                      for g in self.gradients)/len(self.gradients)
        print('l2 norm of gradients before clip', l2_norm)
        if l2_norm < clip:
            return
        mu = torch.sqrt(clip/l2_norm)
        for j, g in enumerate(self.gradients):
            self.gradients[j] *= mu
        l2_norm = sum(g.pow(2.0).mean()
                      for g in self.gradients)/len(self.gradients)
        print('l2 norm of gradients after clip', l2_norm, '\n')
    ...
    def l2_loss(self, l2_lambda, model_index):
        l2_norm = sum(p.pow(2.0).sum()
                      for p in self.models[model_index].parameters())
