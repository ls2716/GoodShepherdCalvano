...
if __name__ == "__main__":
    ...
    outer_agent = CalvanoAgent(0.05, beta=0.99, no_models=30, device=device)
    ...
    for outer_it in tqdm(range(outer_learning_steps)):
        ...
        base_reward = outer_reward + outer_agent.l2_loss(0, model_index=0)
        print(
            f'\n Outer reward is {outer_reward}, inner reward is {inner_reward}')
        print(outer_agent.l2_loss(0.1, model_index=0))
        ...
        for model_index in range(outer_agent.no_models):
            outer_reward, inner_reward = inner_loop(
                ...
                env=env, T=T, device=device)
            rewards.append(
                outer_reward + outer_agent.l2_loss(0, model_index=model_index))

        rewards = torch.tensor(rewards)-base_reward
        outer_agent.calculate_gradients(rewards)
        outer_agent.clip_gradients(0.5)
        outer_agent.update()
